...
class MySprite:
    ...
    def isCollision(self, width: int, height: int, position: tuple):
        """
        ...
        :return: bool
        """
        if self.__x - width < position[0] <= self.__x + self.get_width() and self.__y + self.get_height() >= position[1] >= self.__y - height:
            return True
        else:
            return False
